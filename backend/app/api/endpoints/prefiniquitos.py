...
def sync_prefiniquito_document_on_disk(schema_name: str, pref_id: int, db: Session):
    pref = db.query(Prefiniquito).filter(Prefiniquito.id == pref_id).first()
    if not pref:
        return
    ...
    DocumentService.generate_prefiniquito_excel(export_data, "xlsx", schema_name=schema_name)
...
def export_prefiniquito(
    ...
    filepath = DocumentService.generate_prefiniquito_excel(export_data, out_format, schema_name=schema_name)
    
    empresa_slug = DocumentService._slugify(schema_name if schema_name else export_data.get('razon_social', ''))
    trabajador_slug = DocumentService._slugify(export_data.get('nombre_trabajador', ''))
    m_year = re.search(r'\b(20\d\d)\b', str(export_data.get('fecha_retiro', '')))
    anio = m_year.group(1) if m_year else str(date.today().year)

    master_path = DocumentService.get_prefiniquito_master_path(empresa_slug, anio)
    fecha_act = DocumentService.get_file_last_update_date(master_path)

    if format == "word":
        media_type = "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
        filename = f"prefiniquito_{trabajador_slug}_{empresa_slug}_{anio}_{fecha_act}.docx"
    elif format == "pdf":
        media_type = "application/pdf"
        filename = f"prefiniquito_{trabajador_slug}_{empresa_slug}_{anio}_{fecha_act}.pdf"
    else:
        media_type = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"
        filename = f"prefiniquitos_{empresa_slug}_{anio}_{fecha_act}.xlsx"
