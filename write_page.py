...
export default function PrefiniquitosPage() {
  ...
  useEffect(() => {
    ...
    fetch(http://localhost:8000/api/tenants//employees)
      .then(r => r.json())
      .then(data => {
  ...
  const handleCalculate = async () => {
    ...
    setSuccess(false);
    try {
      const res = await fetch(http://localhost:8000/api/tenants//prefiniquitos/preview, {
        method: "POST",
  ...
  const handleFinalize = async () => {
    ...
    if (!confirm(¿Está seguro de generar el Prefiniquito para ? El empleado pasará a estado INACTIVO.)) return;
    
    setSaving(true);
    try {
      const res = await fetch(http://localhost:8000/api/tenants//prefiniquitos/, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          employee_id: Number(selectedEmpId),
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="space-y-4">
              <div>
                ...
                <select 
                  ...
                  className="w-full border border-slate-300 rounded-xl px-4 py-2.5 bg-slate-50 focus:bg-white"
                >
        ...
        <div className="lg:col-span-8">
          <AnimatePresence>
            {success && (
              <motion.div initial={{opacity:0, y:-10}} animate={{opacity:1, y:0}} className="bg-emerald-50 text-emerald-800 p-4 rounded-xl mb-6 flex items-center gap-3 border border-emerald-200">
            ...
            {calcResult && (
              <motion.div initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} className="bg-white border border-slate-300 rounded-none shadow-lg max-w-3xl mx-auto overflow-hidden">
                <div className="p-12 text-slate-800 text-sm font-mono space-y-6">
                  ...
                  <div className="space-y-1 pb-4 border-b border-slate-300">
                    <div><span className="font-bold">NOMBRE DEL TRABAJADOR:</span> {selectedEmp?.apellidos} {selectedEmp?.nombres}</div>
...
'''
with open("frontend/src/app/(panel)/prefiniquitos/page.tsx", "w", encoding="utf-8") as f:
    f.write(content)